                            ...
                            <div class="lg:w-2/3 w-full mx-auto overflow-auto">
                                <table class="table-auto w-full text-left whitespace-no-wrap">
                                    ...
                                    @foreach ($expired_owners as $expired_owner)
                                        <tr>
                                        <td class="px-4 py-3">{{ $expired_owner->name }}</td>
                                        <td class="px-4 py-3">{{ $expired_owner->age }}</td>
                                        <td class="px-4 py-3">{{ $expired_owner->email }}</td>
                                        <td class="px-4 py-3">{{ $expired_owner->department->name }}</td>
                                        <form id="restore_{{$expired_owner->id}}" method="post" action="{{ route('admin.expired-owners.restore',$expired_owner->id) }}">
                                            @csrf
                                            <td class="px-4 py-3">
                                        ...
                                        <form id="delete_{{$expired_owner->id}}" method="post" action="{{ route('admin.expired-owners.destroy',$expired_owner->id) }}">
                                            @csrf
                                            <td class="px-4 py-3">
