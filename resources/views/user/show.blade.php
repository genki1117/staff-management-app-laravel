                    ...
                    <section class="text-gray-600 body-font relative">
                        <div class="container px-5 mx-auto">
                                ...
                                <div class="ml-16 mx-auto">
                                    ...
                                    <div class="p-2 mx-auto">
                                        <div class="relative">
                                            ...
                                            <p class="text-lg font-semibold">{{ $user->age }}</p>
                                        </div>
                                    </div>
                                    <div class="p-2 mx-auto">
                                        <div class="relative">
                                            ...
                                            {{-- <a href="{{ route('owner.owner_create_mail', $user->id) }}"><i class="fa-solid fa-envelope pt-2 ml-2"></i></a> --}}
                                        </div>
                                    </div>
                                    <div class="p-2 mx-auto">
                                        <div class="relative">
                                            <label for="department_id" class="leading-7 text-sm text-gray-600">部署</label>
                                            <p class="text-lg font-semibold">{{ $user->department->name }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-12 p-2 w-full flex justify-around">
                                <button type="button" onclick="location.href='{{ route('user.users.index') }}'" class="bg-gray-300 border-0 py-2 px-8 focus:outline-none hover:bg-gray-400 rounded text-lg">戻る</button>
                                {{-- <button type="button" onclick="location.href='{{ route('owner.owners.edit', $user->id) }}'" class="text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">編集する</button> --}}
                            </div>
                        </div>
                        {{-- <div class="-mt-10 text-right">
                            <form id="delete_{{$user->id}}" method="post" action="{{ route('owner.owners.destroy', [ 'owner' => $user->id ]) }}">
                                @csrf
                                @method('DELETE')
