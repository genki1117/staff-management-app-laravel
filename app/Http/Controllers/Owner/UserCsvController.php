...
class UserCsvController extends Controller
{
    public function userCsvDownload()
    {
        ...
        $callback = function () {
            ...
            $users = User::select('id', 'name', 'age', 'email', 'department_id', 'created_at')->get(); // tableから情報を取得

            foreach ($users as $row) { // データを1行ずつ回す
                $csv = [
    ...
    public function userCsvUpLoad(CsvUploadRequest $request)
    {
        // User::truncate(); // 全件削除
